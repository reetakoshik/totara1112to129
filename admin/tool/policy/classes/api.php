...
/**
     ...
     */
    public static function list_sub_policies($id = null, $countacceptances = false) {
        global $DB, $PAGE;
        ...
        $sql .= " WHERE v.id IS NOT NULL AND v.parentpolicy =".$id;
    ...
    public static function list_policies($ids = null, $countacceptances = false) {
        ...
        $sql .= " WHERE v.id IS NOT NULL AND v.parentpolicy = '0' ";
    ...
    public static function form_policydoc_data(policy_version $version) {
        ...
        //$commentfieldoptions = static::policy_comment_field_options();
        ...
        if (empty($data->id)) {
            ...
            //$data = file_prepare_standard_editor($data, 'comment', $commentfieldoptions, $commentfieldoptions['context']);
            ...
            //$data = file_prepare_standard_editor($data, 'comment', $commentfieldoptions, $commentfieldoptions['context'],
                //'tool_policy', 'policydocumentcomment', $data->id);
            // $rs = $DB->get_record_sql('SELECT summary,content FROM {tool_policy_versions} WHERE id ='.$data->id);
            
            // $data->summary = $rs->summary;
            // $data->content = $rs->content;
            
        }
//print_r($data); die('summary');
        return $data;
    }
    ...
    public static function form_policydoc_update_new(stdClass $form) {
        ...
        //print_r($form);die();
        $record = new \stdClass();
        $record->policyversionid = $form->id;
        $record->commentext          = $form->comment['text'];
        $record->assignto            = $form->assignto;
        $record->fromto              = $form->fromto;
        $record->cstatus             = $form->cstatus;
        $record->timecreated         = time();
        $record->timemodified        = time();
        $DB->insert_record('tool_policy_comment', $record);
        static::send_policymsg_of_assignchangelog($form->id,$form->name);
       $relate = array();
       if(!empty($form->relatedpolicy)){
        $relate=explode(',', $form->relatedpolicy);
        //print_r($relate);echo"exp456";
        foreach ($relate as $value) {
          $conditions = array('id'=> $value);
          $singlerelate = $DB->get_record('tool_policy_versions', $conditions,'relatedpolicy');
           //print_r($singlerelate);echo"exp123";
           $srelate = array();
          if(!empty($singlerelate->relatedpolicy &&$singlerelate->relatedpolicy!='')){
             //echo $singlerelate->relatedpolicy;
             $srelate=explode(',', $singlerelate->relatedpolicy);
              //print_r($srelate); echo"exp";
             array_push($srelate,$form->id);
            $srelate=array_unique($srelate);
              //print_r($srelate); echo"expadded"; die();
             $valuerelated = implode(",",$srelate);

           } 
           else{
             $valuerelated = $form->id;
           } 
          
          $DB->set_field('tool_policy_versions','relatedpolicy' ,$valuerelated, $conditions);
        }
        }
    ...
    public static function form_policydoc_update_overwrite(stdClass $form) {
       ...
       if(!empty($form->relatedpolicy)){
        ...
        /*$commentfieldoptions = static::policy_comment_field_options();
        $form = file_postupdate_standard_editor($form, 'comment', $commentfieldoptions, $commentfieldoptions['context'],
            'tool_policy', 'policydocumentcomment', $form->id);
        unset($form->comment);
        unset($form->commenttrust);*/

        unset($form->status);
        unset($form->save);
        unset($form->saveasdraft);
        unset($form->minorchange);
        //unset($form->primarylang);
        
        $policyversion = new policy_version($form->id, $form);
        //echo "<pre>";print_r($policyversion);echo "</pre>";die('test111111111111');
     ...
     * @return array
     */
    public static function policy_comment_field_options() {
        global $CFG;
        require_once($CFG->libdir.'/formslib.php');
    ...
    public static function get_agreement_optional($versionid) {
     ...
     public static function form_messages_add_update(stdClass $form) {
        global $DB;

        $object = new \stdClass();
        $object->policyid         = $form->policyid;
        $object->subject          = $form->subject;
        $object->message          = $form->message['text'];
        $object->sendmanager      = $form->sendmanager;
        $object->managersubject   = $form->managersubject;
        $object->managermessage   = $form->managermessage['text'];
        $object->timecreated      = time();
        $object->timemodified     = time();

        if($form->messageid) {
            $object->id         = $form->messageid;
            $DB->update_record('tool_policy_message', $object);
        } else {
            $message= $form->policyid = $DB->insert_record('tool_policy_message', $object);
        }
    } 

    public static function form_messages_data($policyid) {
      global $DB;
     $msgdata = $DB->get_record_sql('SELECT * FROM {tool_policy_message} WHERE policyid ='.$policyid);
     if(!empty($msgdata)) {
        return $msgdata;
    }else{
        $record = new \stdClass();
        $record->subject="subject not set in database";
        $record->message="message not set in database";
        return $record;
    }
     
    }

    public static function send_policymsg_to_audience($policyid) {
        global $DB, $USER;
        $msgcontent = self::form_messages_data($policyid);
        $policyver = $DB->get_record_sql("SELECT relatedaudiences, v.name FROM {tool_policy_versions} v INNER JOIN {tool_policy} p ON p.currentversionid = v.id WHERE p.id = '".$policyid."'");
        $auduserarr = array();
        
        if(!empty($policyver->relatedaudiences)) {
            $audienceids = explode(',', $policyver->relatedaudiences);
            foreach ($audienceids as $aud) {
                $users = $DB->get_records_sql("SELECT id, userid FROM {cohort_members} WHERE cohortid = '".$aud."'");
                foreach($users as $user) {
                    $auduserarr[] = $user->userid;
                }
            }
        }
         
        foreach ($auduserarr as $user) {
            
            $touser = $DB->get_record('user',  array('id' => $user));
           
            $message = new \core\message\message();
            $sendmsg1 = str_replace("%policyname%", '<strong>'.$policyver->name. '</strong> ', $msgcontent->message);
            $cuser = core_user::get_user($USER->id);
            $sendmsg = str_replace("%policymanager%", '<strong>'.$cuser->firstname." ".$cuser->lastname. '</strong> ', $sendmsg1);
            
            //$message = new stdClass();
            $message->component         = 'tool_policy'; //your component name
            $message->name              = 'sendpolicynotification'; //this is the message name from messages.php
            $message->userfrom          = $USER;
            $message->userto            = $touser;
            $message->subject           = $msgcontent->subject;
            $message->fullmessage       = $sendmsg;
            $message->fullmessageformat = FORMAT_PLAIN;
            $message->fullmessagehtml   = $sendmsg;
            $message->smallmessage      = 'Policy message';
            $message->notification      = 1; //this is only set to 0 for personal messages between users
            $a = message_send($message);
        }
    }



  public static function send_policyexpiremsg_to_audience($policyid){
    global $DB, $USER;
    $msgcontent = self::form_messages_data($policyid);
    $policyver = $DB->get_record_sql("SELECT relatedaudiences, v.name FROM {tool_policy_versions} v INNER JOIN {tool_policy} p ON p.currentversionid = v.id WHERE p.id = '".$policyid."'");
        $auduserarr = array();
...
public static function send_policymsg_of_assignchangelog($policyid,$name) {
        global $DB, $USER,$CFG;
       
       //  //$msgcontent = self::form_messages_data($policyid);
        $policyver = $DB->get_record_sql("SELECT v.id,v.policyid FROM {tool_policy_versions} v WHERE v.id = '".$policyid."'");
       // print_r($policyver);
       
        $commentdata = $DB->get_record_sql("SELECT pc.commentext, pc.assignto,pc.fromto, pc.cstatus FROM {tool_policy_comment} AS pc  where pc.policyversionid = '".$policyver->id."' ORDER BY pc.id DESC limit 0,1");
         //print_r($commentdata);
            if($commentdata->cstatus==1){
              $stage='Reviewing';  
            }elseif($commentdata->cstatus==2){
              $stage='Approving';  
            }elseif($commentdata->cstatus==3){
              $stage='Approved';  
            }else{
              $stage='Reviewing';
            }
            $touser = $DB->get_record('user',  array('id' => $commentdata->assignto));
            //print_r($touser);
            if($commentdata->fromto==0){
            $fromuser=$USER; 
            }else {
            $fromuser = $DB->get_record('user',  array('id' => $commentdata->fromto));
            }
            //print_r($fromuser);
           // die('here stop');
            $message = new \core\message\message();
            // $sendmsg1 = str_replace("%policyname%", '<strong>'.$policyver->name. '</strong> ', $msgcontent->message);
             $tousermsg = core_user::get_user($touser->id);
             $fromusermsg= core_user::get_user($fromuser->id);
            $sendmsg = "Hi".$tousermsg->firstname." ".$tousermsg->lastname."
            <br> You have been assigned a policy by ".$fromusermsg->firstname." ".$fromusermsg->lastname." with comment:".$commentdata->commentext." for ".$stage."<br> Please check policy link : <a href='".$CFG->wwwroot."/admin/tool/policy/view.php?policyid=".$policyver->policyid."&versionid=".$policyver->id."&manage=1&returnurl=/admin/tool/policy/managedocs.php>'>".$name."</a>";
            //$message = new stdClass();
            $message->component         = 'tool_policy'; //your component name
            $message->name              = 'sendpolicynotification'; //this is the message name from messages.php
            $message->userfrom          = $USER;
            $message->userto            = $touser;
            $message->subject           = 'policy assignment';
            $message->fullmessage       = $sendmsg;
            $message->fullmessageformat = FORMAT_PLAIN;
            $message->fullmessagehtml   = $sendmsg;
            $message->smallmessage      = 'Policy assignment message';
            $message->notification      = 1; //this is only set to 0 for personal messages between users
            $a = message_send($message);
     ...
     public static function view_policy_search($search){
      global $DB, $USER;
      
      //echo "correct"; 

    $policyidsrelated = $DB->get_records_sql("SELECT DISTINCT pv.id      
       FROM {tool_policy_versions} AS pv 
       ,{tool_policy_versions} as pv2 where (pv.id in (pv2.relatedpolicy)) and (pv2.name like '%".$search."%' or pv2.name like '".$search."%' or pv2.name like '%".$search."')");

    $policyidscourse = $DB->get_records_sql("SELECT DISTINCT pv.id      
       FROM {tool_policy_versions} AS pv 
       INNER JOIN {course} AS cs ON FIND_IN_SET(cs.id,pv.relatedcourse) 
       WHERE  cs.fullname LIKE '".$search."%' 
       OR cs.fullname LIKE '%".$search."' 
       OR cs.fullname LIKE '%".$search."%' ");

    $name =  $DB->get_records_sql("SELECT id FROM  {tool_policy_versions} where name like '".$search."%' OR name like '%".$search."'
        OR name like '%".$search."%' 
    ...
    $authorname =  $DB->get_records_sql("SELECT pv.id FROM {tool_policy_versions} AS pv INNER JOIN {user} AS ms ON pv.usermodified = ms.id where CONCAT(ms.firstname,' ', ms.lastname) LIKE '".$search."' 
        or ms.username like '%".$search."'
        or ms.username like '".$search."%'
        or ms.username like '%".$search."%'
        or ms.firstname like '%".$search."'
        or ms.firstname like '".$search."%'
        or ms.firstname like '%".$search."%'
        or ms.lastname like '%".$search."'
        or ms.lastname like '".$search."%'
        or ms.lastname like '%".$search."%'
        ");
    ...
    $allids = array();
    // print_r("related policy");
    // echo "<br>";
    // print_r($policyidsrelated);
    $allids = array_merge($allids,$policyidsrelated);
    // echo "<br>";
    // print_r("policyidscourse");
    // echo "<br>";
    // print_r($policyidscourse);
    $allids = array_merge($allids,$policyidscourse);
    // echo "<br>";
    // print_r("content");
    // echo "<br>";
    // print_r($name);
    $allids = array_merge($allids,$name);
    // echo "<br>";
    // print_r("authorname");
    // echo "<br>";
    // print_r($authorname);
    $allids = array_merge($allids,$authorname);
    // echo "<br> all id <br>";
    // print_r($allids);
   // die();
// $matchedids = array();
...
//         $currentversionid = $DB->get_records_sql("SELECT currentversionid from {tool_policy}");
//         //print_r($currentversionid);
//         foreach ($currentversionid as $key => $currentversionidvalue) {
//             print_r($currentversionidvalue->currentversionid);

//             if($currentversionidvalue->currentversionid != '')
//             {
//                  $matchedids[] =  $DB->get_records_sql("SELECT id from {tool_policy} where ".$value->id." = ".$currentversionidvalue->currentversionid." "); 
//             }
//             echo "<br>";
//         }
        
       
        // print_r("SELECT id from {tool_policy} where ".$value->id." in (select currentversionid from {tool_policy})");
        // die();    
   // }
     ...
     //print_r($name);
      ...
      public static function apply_policy_filter($filterby)
     {
        //print_r($filterby);
        ...
        if($filterby == 'Status')
        {
                ...
                foreach ($policies as $keypolicy => $valuepolicy)
                {
                    ...
                    if(!empty($valuepolicy->currentversion))
                    {
                        $valuecureent = $valuepolicy->currentversion;
                        //$testcurrent = $valuecureent->id;
                        $active[] = $valuecureent->id;
                    }
                    elseif (!empty($valuepolicy->draftversions)) {
                        # code...
                        $valuedraft = $valuepolicy->draftversions;

                        $draft[] = $valuedraft[0]->id;
                    }
                    else
                    {
                     $valuearchived = $valuepolicy->archivedversions;
                      $archived[] =  $valuearchived[0]->id;
                    }

                }

                  $ids = array_merge($active,$draft);
                  $allids = array_merge($ids,$archived);
                  $filtereddata = $allids;
        }
        elseif($filterby == 'Date')
        {
        //date
        //SELECT * FROM `mdl_tool_policy_versions` ORDER BY `timemodified` DESC
                $filterdata = $DB->get_records_sql("SELECT id FROM {tool_policy_versions} ORDER BY `timemodified` DESC");
                $filterdateids = array();
                foreach ($filterdata as $filterdatekey => $filtervalue) {

                    $filterdateids[] = $filtervalue->id;
                    
                }

                $filtereddata = $filterdateids;
        }

        elseif ($filterby == 'Revision') {
            //revision
                 $filtereddatavalue = $DB->get_records_sql(" SELECT `policyid`, SUM(`archived`) AS sumarchived FROM {tool_policy_versions} GROUP BY `policyid` ORDER BY sumarchived DESC");
                 $filteridrevision = array();
                 foreach ($filtereddatavalue as $filterkey => $filtervalue) {

                    $filteridrevision[] = $DB->get_records_sql(" SELECT `id` FROM {tool_policy_versions} where policyid=".$filtervalue->policyid);
                     
                 }
                // print_r($filteridrevision);
                 $filteridrevsnvalue = array();
                 foreach ($filteridrevision as $filteridkey => $filteridvalue) {

                    foreach ($filteridvalue as $filteridvaluekey => $filteridvaluevalue) {

                        $filteridrevsnvalue[] = $filteridvaluevalue->id;
                        # code...
                    }
                     # code...
                 ...
                 $filtereddata = $filteridrevsnvalue;


                 // echo "filterdata";
                 // print_r($filtereddata);
             // SELECT `policyid`, SUM(`archived`) AS sumarchived FROM mdl_tool_policy_versions GROUP BY `policyid` ORDER BY sumarchived DESC
        }
       ...
       elseif ($filterby == 'Type')
        ...
        $distincttype = $DB->get_records_sql("SELECT DISTINCT `type` FROM {tool_policy_versions}");

       //  print_r($distincttype);
       // die();
          $ids = array();

          foreach ($distincttype as $distincttypekey => $distincttypevalue)
          {
              //print_r($distincttypevalue->type);

             $ids[$distincttypevalue->type] = $DB->get_records_sql("SELECT id,name FROM {tool_policy_versions} where type =".$distincttypevalue->type." ORDER BY name ASC");
          }
           // echo "<pre>";
           // print_r($ids);
          // die();
          ...
          foreach ($ids as $idskey => $idsvalue)
           {
                    foreach ($idsvalue as $idkey => $idvalue)
                     {
                           $keyvlauesid[] = $idvalue->id;  
                     }
                      // print_r($keyvlauesid); 

           }

                     $filtereddata = $keyvlauesid; 

                     // print_r($filtereddata);
                     // die();
        }

        elseif($filterby == 'language' )
        {
            $distinctlang = $DB->get_records_sql("SELECT DISTINCT `primarylang` FROM {tool_policy_versions}");
            $langids = array();
            $policies = api::list_policies();
            $en = array();
            $he = array();
              ...
              foreach ($policies as $keypolicy => $valuepolicy)
                {
                    if(!empty($valuepolicy->currentversion))
                    {
                        ...
                        $curentlangvalue = $valuecureent->primarylang;

                        if($curentlangvalue == 'en')
                        {
                            $en[] = $valuecureent->id;
                        }
                        if($curentlangvalue == 'he')
                        {
                            $he[] = $valuecureent->id;
                        }
                    }
                    elseif (!empty($valuepolicy->draftversions)) {
                        # code...
                        $valuedraft = $valuepolicy->draftversions;
                        $curentlangvalue = $valuedraft[0]->primarylang;
                         if($curentlangvalue == 'en')
                        {
                            $en[] =$valuedraft[0]->id;
                        }
                    ...
                    {
                     $valuearchived = $valuepolicy->archivedversions;
                     $curentlangvalue = $valuearchived[0]->primarylang;
                      if($curentlangvalue == 'en')
                        {
                            $en[] =$valuearchived[0]->id;
                        }
                        if($curentlangvalue == 'he')
                        {
                            $he[] =$valuearchived[0]->id;
                        }
                    }
                }

                $langids = array_merge($en,$he);
                 
            $filtereddata = $langids;
        }

        elseif($filterby == 'Completion')
        {            
        ...
        foreach ($policies as $i => $policy) {
            ...
            if (!empty($policy->archivedversions)) {
                // There is no current and no draft versions, display the first archived version.
                //print_r($policy->archivedversions); die('heloreee');

                $percentvalue = api::completionvalue(array_shift($policy->archivedversions));
                if(!empty($percentvalue->subpercentageVal))
                {
                    if(!empty($policy->archivedversions[0]->id))
                    {
               $firstpolicy[$policy->archivedversions[0]->id] = $percentvalue->subpercentageVal;
                    }
                }
            }}
            ...
            if (!empty($policy->currentversion)) {
               ...
               $percentvalue = api::completionvalue($policy->currentversion);
               if(!empty($percentvalue->subpercentageVal))
                {
               $firstpolicy[$policy->currentversion->id] = $percentvalue->subpercentageVal;
                }
            } 
        }  
        arsort($firstpolicy);
        $policycompids = array();
        foreach ($firstpolicy as $firstpolicykey => $firstpolicyvalue) {
            $policycompids[] = $firstpolicykey;
        }
        $policycompids = array_merge($policycompids,$compids);

        $filtereddata = $policycompids;
        }    
    
        return $filtereddata;
     }

     public static function completionvalue($version)
     {
        $subpercent= new \stdClass();
        $subpercent->subpercentages = $version->acceptancescounttext;
