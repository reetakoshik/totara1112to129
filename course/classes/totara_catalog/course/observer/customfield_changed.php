...
class customfield_changed {
    ...
    /**
     * Process the adding or updating custom field. update course catalog data based on custom field default values
     *
    ...
    public static function update_default_data(base_event $event): void {
        ...
        if ($eventdata['other']['type'] == 'course' &&
            !empty($eventdata['other']['data']['defaultdata'])
        ) {
            $adhoctask = new provider_active_task();
            $adhoctask->set_custom_data(array('objecttype' => 'course'));
