...
/**
 * golearningzone is a clean and customizable theme.
 *
 * @package     theme_golearningzone
...
class theme_golearningzone_core_course_renderer extends core_course_renderer {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        $this->enablecategoryicon = \theme_golearningzone\toolbox::get_setting('enablecategoryicon');
    }
    ...
    protected function coursecat_category(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        if ($coursecat->get_children_count()) {
            ...
            $coursecounttitle = get_string('numberofcoursesandsubcatcourses', 'theme_golearningzone');
        } else {
        ...
        if (\theme_golearningzone\toolbox::get_setting('enablecustomcategoryicon')) {
            // User may have set a value for the category.
            $image = \theme_golearningzone\toolbox::get_setting('categoryimage'.$coursecat->id, 'format_file_url');
            if (empty($image)) {
                $icon = \theme_golearningzone\toolbox::get_setting('categoryicon'.$coursecat->id);;
            }
        }
        if ((empty($icon)) && (empty($image))) {
            // User hasn't set a value for the category, get the default.
            $image = \theme_golearningzone\toolbox::get_setting('defaultcategoryimage', 'format_file_url');
            if (empty($image)) {
                $icon = \theme_golearningzone\toolbox::get_setting('defaultcategoryicon');
            }
        }
        if (!empty($image)) {
            ...
            $categoryrepresentation = \theme_golearningzone\toolbox::getfontawesomemarkup($icon);
        } else {
    ...
    protected function coursecat_coursebox_content(coursecat_helper $chelper, $course) {
        ...
        if ($coursehascontacts) {
            ...
            foreach ($course->get_course_contacts() as $userid => $coursecontact) {
                $faiconsetting = \theme_golearningzone\toolbox::get_setting('courselistteachericon');
    ...
    /**
     * Serves requests to /theme/golearningzone/inspector.ajax.php
     *
    ...
    public function inspector_ajax($term) {
        ...
        if (empty($courses)) {
            $tallystr = get_string('tallyoftotal', 'theme_golearningzone', array('tally' => 0, 'total' => 0));
            $data[] = array('label' => $tallystr, 'value' => '');
            return $data;
        }

        if ($this->get_setting('searchallcoursecontentenable')) {
            $courseitemsearchtype = \get_user_preferences('theme_golearningzone_courseitemsearchtype');
        } else {
            $courseitemsearchtype = \theme_golearningzone\toolbox::get_setting('searchallcoursecontentdefault');
        }
        ...
        if ($courseitemsearchtype) {
            $tallystr = get_string('tallyinoftotal', 'theme_golearningzone', array('rtally' => count($data), 'ctally' => $coursetally, 'ctotal' => count($courses)));
        } else {
            $tallystr = get_string('tallyoftotal', 'theme_golearningzone', array('tally' => $coursetally, 'total' => count($courses)));
        }
        $data[] = array('label' => $tallystr, 'value' => '');
        return $data;
    }
