...
/**
 * Handles certificate issue user data.
 */
class certificate_issues extends \totara_userdata\userdata\item {
    ...
    public static function get_fullname_string() {
        return ['userdata_certificate_issues', 'mod_certificate'];
    }
    ...
    protected static function purge(target_user $user, \context $context) {
        ...
        $join = self::get_activities_join($context, 'certificate', 'ci.certificateid', 'c');

        $sql = "SELECT ci.id, ctx.id AS contextid
        ...
        foreach ($records as $record) {
            $fs->delete_area_files($record->contextid, 'mod_certificate', 'issue', $record->id);
            $DB->delete_records('certificate_issues', ['id' => $record->id]);
        }
    ...
    protected static function export(target_user $user, \context $context) {
        ...
        $sql = "SELECT ci.*, c.name, c.intro, ctx.id AS contextid
                FROM {certificate_issues} ci
                {$join}
                WHERE ci.userid = :userid";
        $params = ['userid' => $user->id];
        ...
        foreach ($records as $record) {
            ...
            $files = $fs->get_area_files($record['contextid'], 'mod_certificate', 'issue', $record['id'], null, false);
