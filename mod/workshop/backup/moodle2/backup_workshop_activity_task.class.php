...
class backup_workshop_activity_task extends backup_activity_task {
    ...
    static public function encode_content_links($content, backup_task $task = null) {
        ...
        if (!self::has_scripts_in_content($content, 'mod/workshop', ['index.php', 'view.php'])) {
        ...
        if (empty($task)) {
            ...
            $content = self::encode_content_link_basic_id($content, "/mod/workshop/index.php?id=", 'WORKSHOPINDEX');
            $content = self::encode_content_link_basic_id($content, "/mod/workshop/view.php?id=", 'WORKSHOPVIEWBYID');
        } else {
            // OK we have a valid task, we can translate just those links belonging to content that is being backed up.
            $content = self::encode_content_link_basic_id($content, "/mod/workshop/index.php?id=", 'WORKSHOPINDEX', $task->get_courseid());
            foreach ($task->get_tasks_of_type_in_plan('backup_workshop_activity_task') as $task) {
                /** @var backup_workshop_activity_task $task */
                $content = self::encode_content_link_basic_id($content, "/mod/workshop/view.php?id=", 'WORKSHOPVIEWBYID', $task->get_moduleid());
            }
        }
