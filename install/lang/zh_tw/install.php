...
$string['pathssubdataroot'] = '<p>你需要有一個目錄讓Totara可以儲存所有的用戶上傳的檔案。</p><p>這目錄對於網頁伺服器用戶(通常是"www-data"、"nobody"或"apache")而言，應該是可讀的和<b>可寫的</b>。</p>
<p>它必須是不能透過網際網路直接存取的。</p>
<p>若此目錄目前不存在，這安裝過程將會試著建立它。</p>';
$string['pathssubdirroot'] = '包含Totara程式碼的目錄的完整路徑';
$string['pathssubwwwroot'] = '可以進入使用Totara的完整網址，也就是用戶為了要使用Totara，而需要輸入到瀏覽器的網址列的地址。

不可能使用多個網址來存取Totara，如果您的網站有多個公開網址，您必須選擇一個最簡單的網址，並把其他的網址都設定為永久重新導向。
...
$string['welcomep30'] = '這一<strong>{$a->installername}</strong>的版本包含了一些可以建立<strong>Totara</strong>執行環境的應用程序：';
...
$string['welcomep60'] = '接下來的一些頁面將會透過一些簡單的步驟引導您配置和設定在你電腦上的 <strong>Totara</strong> 。
