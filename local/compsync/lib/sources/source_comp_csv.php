...
require_once($CFG->dirroot.'/local/compsync/lib/sources/classes/source.comp.class.php');
...
class totara_sync_source_comp_csv extends totara_sync_source_comp {
    ...
    function config_form(&$mform) {
        ...
        $mform->addElement('select', 'csvcompencoding', get_string('csvencoding', 'tool_totara_sync'), $encodings);
        $mform->setType('csvcompencoding', PARAM_ALPHANUMEXT);
        $default = $this->get_config('csvcompencoding');
        $default = (!empty($default) ? $default : 'UTF-8');
        $mform->setDefault('csvcompencoding', $default);
    ...
    function config_save($data) {
        ...
        $this->set_config('csvcompencoding', $data->{'csvcompencoding'});
    ...
    function import_data($temptable) {
        ...
        if ($fileaccess == FILE_ACCESS_DIRECTORY) {
            ...
            $fieldid = get_config('totara_sync', 'sync_comp_itemid');

            // Check the file exists
            if (!$fs->file_exists($systemcontext->id, 'totara_sync', 'comp', $fieldid, '/', '')) {
            ...
            // Get the file
            $fsfiles = $fs->get_area_files($systemcontext->id, 'totara_sync', 'comp', $fieldid, 'id DESC', false);
            ...
            $fsfile->copy_content_to($tempdir.'/comp.php');
            $itemid = $fsfile->get_itemid();
            $fs->delete_area_files($systemcontext->id, 'totara_sync', 'comp', $itemid);
            $storefilepath = $tempdir.'/comp.php';
        }

        $encoding = $this->get_config('csvcompencoding');
        ...
        while ($row = fgetcsv($file, 0, $this->config->delimiter)) {
            ...
            $row['aggregationmethod'] = !empty($row['aggregationmethod']) ? $row['aggregationmethod'] : 1;
            $row['proficiencyexpected'] = !empty($row['proficiencyexpected']) ? $row['proficiencyexpected'] : 1;
            $row['evidencecount'] = !empty($row['evidencecount']) ? $row['evidencecount'] : 0;
            ...
            if (!empty($customfieldkeys)) {
                ...
                foreach ($customfieldkeys as $key) {
                    ...
                    if (!empty($value)) {
                        ...
                        $datatype = $DB->get_field('comp_type_info_field', 'datatype', array('shortname' => $shortname));
                        switch ($datatype) {
                            case 'datetime':
                                // Try to parse the contents - if parse fails assume a unix timestamp and leave unchanged
