...
/**
 * Goal custom field management class.
 *
 * @package totara_hierarchy
 * @category totara_customfield
 */
class goal implements \totara_customfield\area {
     ...
     *
     * Access copied from totara/hierarchy/item/view.php
     *
    ...
    public static function can_view($instanceorid) {
        ...
        $perms = $hierarchy->get_permissions(null, $USER->id);

        return ($perms['canview'] || $perms['can_view_company']);
    }
    ...
    public static function pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options = array()) {
        ...
        $sql = "SELECT g.*
                  FROM {goal_type_info_data} gid
                  JOIN {goal} g ON g.id = gid.goalid
                 WHERE gid.id = :id";
        $params = array(
            'id' => reset($args)
        );
