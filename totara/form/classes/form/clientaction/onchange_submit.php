...
/**
 * Onchange submit client action class.
 *
 * The onchange submit client action submits the form when the targetted element changes.
 *
 ...
 * @author Brian Barnes <brian.barnes@totaralearning.com>
 */
class onchange_submit implements clientaction {
    ...
    /**
     * Autosubmit client action constructor.
     *
    ...
    public function __construct(item $target) {
        if (!$target instanceof supports_onchange_clientactions) {
            throw new \coding_exception('Only elements implementing \totara_form\form\clientaction\supports_onchange_clientactions can use the onchange submit client action.', get_class($target));
        }
        $this->target = $target;
    }
     ...
     *
     * If the value of an element using this client action changes to an ignored value then the form will NOT be submit.
     *
    ...
    /**
     * Adds empty value to the list of values to ignore when checking if the form should be submit.
     *
     * If the value of an element using this client action changes to an ignored value then the form will NOT be submit.
     *
     * @return self This method is chainable.
