...
namespace totara_certification;
...
class plugininfo extends \core\plugininfo\totara {
    public function get_usage_for_registration_data() {
        global $DB, $CFG;
        $data = array();
        $data['numcertifications'] = $DB->count_records_select('prog', 'certifid IS NOT NULL');
        ...
        $data['certificationsenabled'] = (int)!empty($CFG->enablecertifications);
