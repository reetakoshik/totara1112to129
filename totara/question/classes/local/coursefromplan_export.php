 ...
 * @package totara_question
 */
class coursefromplan_export extends review_export {
    ...
    public function get_items(int $questfieldid, int $answerfieldid) : array {
        ...
        $sql =
            "SELECT DISTINCT {$key} AS id, course.fullname AS name
               FROM {{$prefix}_review_data} rd
               JOIN {dp_plan_course_assign} pca ON rd.itemid = pca.id
               JOIN {course} course ON pca.courseid = course.id
