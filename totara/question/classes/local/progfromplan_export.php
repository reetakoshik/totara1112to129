 ...
 * @package totara_question
 */
class progfromplan_export extends review_export {
    ...
    public function get_items(int $questfieldid, int $answerfieldid) : array {
        ...
        $sql =
            "SELECT DISTINCT {$key} AS id, prog.fullname AS name
               FROM {{$prefix}_review_data} rd
               JOIN {dp_plan_program_assign} ppa on rd.itemid = ppa.id
               JOIN {prog} prog ON ppa.programid = prog.id
