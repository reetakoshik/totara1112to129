...
Y.namespace('M.atto_collapse').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _setVisibility: function(button, visibility) {
        ...
        // Why are we looping with setTimeout?
        // because the image / icon is rendered asynchronously
        // (lib/editor/atto/yui/src/editor/js/editor-plugin-buttons.js around line 215)
        // and it is unlikely to be ready by the time _setVisibility is called.
        var set_accessible_text = function() {
            if (button.one('img')) {
                button.one('img').set('alt', button.get('title'));
            } else if (button.one('.sr-only')) {
                button.one('.sr-only').setContent(button.get('title'));
            } else {
                setTimeout(set_accessible_text, 100);
            }
        };
